...
According to our alternative hypothesis (H1), which question are we asking? We didn't state if we think the cookie machine might be producing more or less than specification, just if it's producing something other than the manufacturer specified average.
