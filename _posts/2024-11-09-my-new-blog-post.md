...
## Conclusion
Both models have essentially the same summary statistics. However, the first (full) model does not have singularities, while the second model (interaction term only) does have singularities. The models also only seem to have moderate explanatory power.
